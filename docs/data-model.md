...
없다 ([[decisions#adr-008]]).

**트랙 분리 원칙**: 같은 `org_name` 안에서 한 experience 의 날짜 범위가 다른 experience 의 날짜
범위에 완전히 포함되면(예: 자람 학회의 "회원" 기간 안에 "임원진(총무)" 기간이 포함) 프론트엔드가
이를 "같은 트랙 안의 오버레이"로 자동 인식해 겹쳐 그린다. 반대로 `org_name` 이 다르면 날짜 범위가
겹치더라도(예: 한양대학교 재학 기간과 대한민국 육군 복무 기간) 항상 별도 트랙으로 분리해서 그린다 —
소속이 다르면 "포함 관계"가 아니라 "같은 시기에 벌어진 별개의 일"이기 때문이다. 별도의 부모-자식
FK 컬럼 없이 `org_name` 일치 여부와 날짜만으로 판단한다.
